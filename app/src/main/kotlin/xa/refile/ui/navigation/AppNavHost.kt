...
import xa.refile.ui.settings.FileSettingsScreen
...
object Routes {
    ...
    /** 文件设置子页路由（冲突策略/回收站/并发）。从设置页「文件」分组跳转。 */
    ...
    /** 文件设置子页跳转串。 */
    fun fileSettings(): String = FILE_SETTINGS
}
...
fun AppNavHost(navController: NavHostController = rememberNavController()) {
    ...
    NavHost(
        ...
        // 性能修复：Navigation 2.9 默认转场为 slide + fade（~220-440ms）。popBackStack 时
        // 退出页在转场期间叠加于目标页之上并拦截其 pointer 输入，表现为「返回后列表已显示
        // 但约 1 秒内点击无响应」（如设置页返回服务器列表）。将退出动画置空，退出页立即移除，
        // 目标页即时可点；进入侧保留极短 fade 兼顾过渡感（fade 不阻塞输入）。
        ...
        composable(
            ...
            // 重命名完成后从进度页返回时，savedStateHandle["should_refresh"]=true，
            // 触发浏览器刷新当前目录以展示最新文件列表。
        ...
        composable(
            ...
            PreviewScreen(
                ...
                matchSessionVm = sessionVm,
                onBack = {
                    // 返回上一页；Preview 正上方一定是 Browser（不存在中间页），
                    // 直接 popBackStack() 最可靠，避免 route pattern 匹配失败导致跳回 Servers
        ...
        composable(
            ...
            ProgressScreen(
                onBackHome = {
                    // 重命名完成后：返回文件浏览器（保留之前浏览的目录）并触发刷新，
                    // 而非回到服务器列表首页。若 browser 不在回退栈（如进程被杀恢复后），
                    // 回退到服务器列表兜底。
                    try {
                        val browserEntry = navController.getBackStackEntry(Routes.BROWSER_ROUTE)
                        browserEntry.savedStateHandle["should_refresh"] = true
                        navController.popBackStack(Routes.BROWSER_ROUTE, inclusive = false)
        ...
        composable(Routes.SETTINGS) {
            SettingsScreen(
                ...
                onOpenFileSettings = { navController.navigate(Routes.fileSettings()) },
            )
        }
        ...
        composable(Routes.FILE_SETTINGS) {
            FileSettingsScreen(onBack = { navController.popBackStack() })
        }
    }
}
