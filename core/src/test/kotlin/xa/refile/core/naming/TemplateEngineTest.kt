...
class TemplateEngineTest {
    ...
    @Test fun `default preset episode template`() {
        ...
        assertThat(r.path).isEqualTo("Firefly(2002)S01E01—Serenity")
    }
    ...
    @Test fun `default preset movie template`() {
        ...
        assertThat(r.path).isEqualTo("Avatar (2009)")
    }
    ...
    @Test fun `clean strips brackets separators and clutter`() {
        // clean() 语义：剥离括号组（含内容）+ 分隔符归一 + 折叠空白
        // [ONe]_Ano_Hana_01_(1280x720) ➔ Ano Hana 01
        val r = engine(media = MediaMetadata(name = "[ONe]_Ano_Hana_01_(1280x720)"))
            .render("{n.clean()}")
        assertThat(r.path).isEqualTo("Ano Hana 01")
    }
    ...
    @Test fun `zone converts epoch to given timezone`() {
        // 1700000000 = 2023-11-14T22:13:20Z；UTC 为 14 日，Asia/Shanghai(+08:00) 为 15 日
        val m = MediaMetadata(info = mapOf("ts" to "1700000000"))
        val utc = engine(media = m).render("{info.ts|zone('UTC')|format('yyyy-MM-dd')}")
        assertThat(utc.path).isEqualTo("2023-11-14")
        val sh = engine(media = m).render("{info.ts|zone('Asia/Shanghai')|format('yyyy-MM-dd')}")
        assertThat(sh.path).isEqualTo("2023-11-15")
    }

    @Test fun `zone invalid zone id returns value unchanged`() {
        val r = engine(media = MediaMetadata(info = mapOf("ts" to "1700000000")))
            .render("{info.ts|zone('Not/AZone')}")
        // 无效 zone：原样返回 epoch 字符串
        assertThat(r.path).isEqualTo("1700000000")
    }

    // ---- 路径操作修饰符 ----

    @Test fun `div concatenates path segments`() {
        val r = engine(media = MediaMetadata(name = "library")).render("{n|div('Movies', '2023')}")
        assertThat(r.path).isEqualTo("library/Movies/2023")
    }

    @Test fun `plus concatenates string not path separator`() {
        // plus 是字符串 concat（非路径分隔拼接），语义不同于 div
        val r = engine(media = MediaMetadata(name = "Avatar")).render("{n|plus('.bak')}")
        assertThat(r.path).isEqualTo("Avatar.bak")
    }

    @Test fun `mod inserts suffix before extension keeping parent`() {
        // "Avatar (2009).mp4"|mod(' [720p]') ➔ "Avatar (2009) [720p].mp4"
        val r = engine(media = MediaMetadata(name = "Avatar (2009).mp4")).render("{n|mod(' [720p]')}")
        assertThat(r.path).isEqualTo("Avatar (2009) [720p].mp4")
    }

    @Test fun `mod without extension appends suffix`() {
        val r = engine(media = MediaMetadata(name = "Avatar")).render("{n|mod(' [720p]')}")
        assertThat(r.path).isEqualTo("Avatar [720p]")
    }

    @Test fun `getRoot returns first segment`() {
        val r = engine(media = MediaMetadata(name = "library/Movies/Anime")).render("{n|getRoot}")
        assertThat(r.path).isEqualTo("library")
    }

    @Test fun `getTail drops first segment`() {
        val r = engine(media = MediaMetadata(name = "library/Movies/Anime")).render("{n|getTail}")
        assertThat(r.path).isEqualTo("Movies/Anime")
    }

    @Test fun `head returns nth segment one indexed`() {
        val r = engine(media = MediaMetadata(name = "library/Movies/Anime")).render("{n|head(2)}")
        assertThat(r.path).isEqualTo("Movies")
    }

    @Test fun `head beyond size returns original`() {
        val r = engine(media = MediaMetadata(name = "library/Movies")).render("{n|head(5)}")
        assertThat(r.path).isEqualTo("library/Movies")
    }

    @Test fun `tail returns last n segments joined`() {
        val r = engine(media = MediaMetadata(name = "library/Movies/Anime")).render("{n|tail(2)}")
        assertThat(r.path).isEqualTo("Movies/Anime")
    }

    @Test fun `listPath returns all segments`() {
        val r = engine(media = MediaMetadata(name = "library/Movies/Anime")).render("{n|listPath}")
        assertThat(r.path).isEqualTo("library, Movies, Anime")
    }

    @Test fun `getRelativePathTail returns last n segments`() {
        val r = engine(media = MediaMetadata(name = "library/Movies/Anime")).render("{n|getRelativePathTail(1)}")
        assertThat(r.path).isEqualTo("Anime")
    }

    @Test fun `toFile normalizes double slashes`() {
        val r = engine(media = MediaMetadata(name = "a//b///c")).render("{n|toFile}")
        assertThat(r.path).isEqualTo("a/b/c")
    }
}
