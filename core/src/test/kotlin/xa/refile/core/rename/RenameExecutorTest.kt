...
class RenameExecutorTest {
    ...
    /**
     * B9 修复：safeDelete 把 /Movies/a.mkv 移到源文件父目录下的 .trash：
     * 先 MKCOL /Movies/.trash，再 MOVE /Movies/a.mkv → /Movies/.trash/a.mkv。
     * 不跨目录移动，回收站始终在源文件所在目录内。
     */
    @Test fun `safeDelete moves file to trash dir mirroring structure`() = runTest {
        server.enqueue(MockResponse().setResponseCode(201)) // MKCOL /Movies/.trash
        server.enqueue(MockResponse().setResponseCode(201)) // MOVE /Movies/a.mkv → /Movies/.trash/a.mkv
        ...
        assertThat(requests).hasSize(2)
        assertThat(requests[0].method).isEqualTo("MKCOL")
        assertThat(requests[0].path).isEqualTo("/Movies/.trash")
        assertThat(requests[1].method).isEqualTo("MOVE")
        assertThat(requests[1].path).isEqualTo("/Movies/a.mkv")
        // 覆盖模式：不发送 Overwrite: F
        assertThat(requests[1].getHeader("Overwrite")).isNotEqualTo("F")
    }
