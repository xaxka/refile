...
class TemplateEngine(
     ...
     * Feature #21：管道表达式编译缓存。
     *
     * P7 修复：改为 companion object 级静态缓存，跨 TemplateEngine 实例共享。
     * 原实现每次新建 TemplateEngine 实例时 expressionCache 为空，同一表达式（如 `n.clean()`）
     * 会重复编译。改为静态后同模板渲染不同文件时只编译一次。
     *
     * [ConcurrentHashMap] 保证并发安全；key 为原始表达式字符串，value 为编译后的管道链。
     */
    private val expressionCache get() = STATIC_EXPRESSION_CACHE
    ...
    /**
     * 从引号起始位置跳到闭合引号后一位，处理 `''` / `""` 转义。
     * 返回闭合引号之后的索引；若未找到闭合引号则返回 s.length。
     */
    ...
    companion object {
        /** P7：跨实例共享的表达式编译缓存。 */
        private val STATIC_EXPRESSION_CACHE = ConcurrentHashMap<String, CompiledExpression>()
    }
}
