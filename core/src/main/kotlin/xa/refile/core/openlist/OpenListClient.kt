...
class OpenListClient internal constructor(
    ...
    /**
     * 建目录（幂等）。成功（200）返回 true；已存在（500 含 "exist"）视为幂等成功返回 true；
     * 其余失败返回 false。
     */
    override suspend fun mkcol(path: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            // 500 "object already exists" → 幂等成功（对齐 WebDAV MKCOL 405 语义）。
            e.code == 500 && e.message.contains("exist", ignoreCase = true)
        } catch (e: IOException) {
            false
        }
    }
