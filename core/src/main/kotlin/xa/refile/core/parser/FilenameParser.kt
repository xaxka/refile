 ...
 * - P0.4：季号 sanity 上限（[MAX_SEASON]）；集号不设上限（国产剧常 50+ 集/季，长篇动画可达千集）。
...
class FilenameParser {
    ...
    /**
     * P1 优化：parse 结果缓存（LRU），避免批量匹配时同名文件重复解析。
     * 容量 256 足够覆盖一批文件中的重复标题变体。
     * 线程安全：用 synchronized 保护（FilenameParser 实例在 MatchViewModel 中被多协程共享）。
     */
    private val parseCache = object : LinkedHashMap<String, ParsedFilename>(64, 0.75f, true) {
        override fun removeEldestEntry(eldest: Map.Entry<String, ParsedFilename>): Boolean = size > 256
    }
    private val cacheLock = Any()

    /** 视频扩展名（不区分大小写），见 §5.2。iso 默认不参与重命名但可解析。
     *  mk3d/mks/mka 为 Matroska 系列衍生（3D / 字幕流 / 音频流），ogm 为 Ogg Media 容器，
     *  strm/strmlnk 为流媒体指向文件（文本条目，按视频库管理）。 */
    ...
    fun parse(fileName: String): ParsedFilename {
        ...
        // P1 优化：先查缓存，命中则跳过全部正则解析。
        synchronized(cacheLock) { parseCache[raw]?.let { return it } }
        val result = parseInternal(raw)
        synchronized(cacheLock) { parseCache[raw] = result }
        return result
    }

    private fun parseInternal(raw: String): ParsedFilename {
        // 1. 去扩展名
        ...
        //    全括号命名兜底：当 cleaned 为空但 brackets 有内容时，从 brackets 恢复标题
        //    过滤已知停用词/短标签/组名/CRC，避免 [国漫]/[CHS]/[GM-Team] 污染标题
        val effectiveCleaned = if (tokens.cleaned.isBlank() && tokens.brackets.isNotEmpty()) {
            tokens.brackets
                .filter { it.trim() != tokens.group }
                .filter { bracket -> isTitleBracket(bracket.trim()) }
                .joinToString(" ")
        } else {
            tokens.cleaned
        }
        val spaced = normalizeSeparators(effectiveCleaned).trim()
        // 4. 解析技术标签
        val tech = parseTech(baseName)
     ...
     * P3.0 重构：不再固定第一=组名/第二=标题/第三=集号的位置约束，改为灵活扫描所有方括号：
     * - 组名：第一项匹配 [GROUP_TOKEN] 的 bracket
     * - 集号：第一项匹配 [BRACKET_EP] 的纯数字 bracket（1-4 位，支持 One Piece 1000+）
     * - 标题：组名与集号之间所有非标签 bracket 拼接；无集号时取所有非组名非标签 bracket
     * - 标签过滤：匹配 [ReleaseInfoDictionary.HARD_STOPWORDS] / 短 CRC / 纯大写代码的 bracket 跳过
     */
    private fun tryAnimePattern(baseName: String): ParsedFilename? {
        ...
        // 关键守卫：如果方括号之间有实质文本（非纯括号命名），退回通用路径。
        // 忽略括号内 CRC（如 (00B82A9E)）和文件扩展名（.mkv/.mp4），
        // 仅当剩余部分有实际标题文本（如 [Erai-raws] Yuusha no Kuzu - 24 [1080p]）时才退回。
        val textOutsideBrackets = ANIME_BRACKETS.replace(baseName, " ")
            .replace(Regex("\\([^)]*\\)"), " ")   // 去除圆括号内容（CRC）
            .replace(Regex("\\.[a-zA-Z0-9]{2,7}$"), " ")  // 去除扩展名
            .trim()
        if (textOutsideBrackets.isNotBlank()) return null

        // 找到组名 bracket：第一个匹配 GROUP_TOKEN 的
        val groupIdx = contents.indexOfFirst { GROUP_TOKEN.matches(it.trim()) }
        val group = if (groupIdx >= 0) contents[groupIdx].trim() else null

        // 找到集号 bracket：第一个纯数字（1-4 位）
        val epIdx = contents.indexOfFirst { ANIME_EP_NUM.matches(it.trim()) }

        if (epIdx < 0) {
            // 无集号：从非组名、非标签 bracket 中拼标题（适用于 season pack / movie）
            val titleParts = contents.filterIndexed { idx, c ->
                idx != groupIdx && isTitleBracket(c.trim())
            }.map { it.trim() }
            if (titleParts.isEmpty()) return null
            val rawTitle = titleParts.joinToString(" ")
            // 拆分中英混合标题（天气之子 Weathering With You → title + alias）
            val (primaryTitle, aliases) = splitTitleAliases(rawTitle)
            val tech = parseTech(baseName)
            return ParsedFilename(
                title = primaryTitle,
        ...
        // 有集号：标题在组名和集号之间（过滤标签），标记 isAbsoluteEpisode
        val ep = contents[epIdx].trim().toInt()
        val titleStart = if (groupIdx >= 0 && groupIdx + 1 < epIdx) groupIdx + 1 else 0
        val titleParts = contents.subList(titleStart, epIdx)
            .filter { c -> isTitleBracket(c.trim()) }
            .map { it.trim() }
        if (titleParts.isEmpty()) return null
    ...
    /** 判断 bracket 内容是否为标题候选（非标签/组名/CRC/技术词/纯数字）。 */
    private fun isTitleBracket(t: String): Boolean {
        if (t.isBlank()) return false
        val lower = t.lowercase()
        // 排除已知技术停用词（含 国漫/简繁/CHS 等标签）
        if (ReleaseInfoDictionary.isHardStopword(t)) return false
        // 排除短标签：纯大写/数字/连字符 2-4 字符（CHS/JP/x264 等）
        if (t.length in 2..4 && lower.all { it in 'a'..'z' || it.isDigit() || it == '-' }) return false
        // 排除 CRC 风格 8 位 hex
        if (t.length == 8 && t.all { it in '0'..'9' || it in 'A'..'F' || it in 'a'..'f' }) return false
        // 排除纯数字（集号）
     ...
     * - episodes：不限制集数上限（国产剧常 50+ 集/季，长篇动画可达千集）；
     *   仅校验非负（>0），任一 ≤0 则整组置空。
    ...
    private fun sanitizeSeasonEpisode(result: SeasonEpisodeResult): SeasonEpisodeResult {
        ...
        // 集号不再设上限：国产剧常 50+ 集/季，One Piece 等长篇动画绝对集号可达千集。
        // 仅丢弃 ≤0 的非法值；E100+ 同时标记为绝对集号（影响 absolute_episode 解析）。
        val hasLargeEp = result.episodes.any { it >= 100 }
        val episodes = if (result.episodes.isNotEmpty() && result.episodes.all { it > 0 }) {
            result.episodes
        } else {
            emptyList()
        }
    ...
    private fun parse3D(input: String): String? {
        // 3D 格式双重匹配：precedingToken + token
    ...
    companion object {
        ...
        // Anime 全括号命名中的集号（1-4 位，支持 One Piece / 长篇动画 1000+ 集）
        private val ANIME_EP_NUM = Regex("^\\d{1,4}$")
        ...
        // 允许连字符以适应 GM-Team / VCB-Studio 等常见组名格式
        private val GROUP_TOKEN = Regex("^[A-Za-z0-9][A-Za-z0-9-]*[A-Za-z0-9]$")
