...
class WebDavClient(
    ...
    // B1 修复：client 为 null 时延迟到首次请求时才建 OkHttpClient（避免每次实例化新建独立连接池）。
    // 调用方（ServerRepository.clientFor）应传入共享 client 以复用连接池/线程池。
    private val httpClient: OkHttpClient = (client ?: DEFAULT_SHARED_CLIENT).newBuilder()
    ...
    companion object {
        // B1 修复：未注入 client 时的共享兜底 client（复用同一连接池/线程池，避免每次实例化新建）。
